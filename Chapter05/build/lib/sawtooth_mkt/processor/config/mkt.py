...
def load_default_mkt_config():
    """
    Returns the default MktConfig
    """
    return MktConfig(
...
def load_toml_mkt_config(filename):
    """Returns a MktConfig created by loading a TOML file from the
    filesystem.
    ...
    Returns:
        config (MKTConfig): The MKTConfig created from the stored
            toml file.
...
def merge_mkt_config(configs):
    """
    Given a list of MktConfig objects, merges them into a single
    MktConfig, giving priority in the order of the configs
    (first has highest priority).

    Args:
        config (list of MktConfigs): The list of mkt configs that
            must be merged together

    Returns:
        config (MktConfig): One MktConfig that combines all of the
            passed in configs.
    """
    connect = None
